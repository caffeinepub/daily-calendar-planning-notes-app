import { DayPlansMap, DayPlan } from './types';
...
export function getDayPlan(date: string, dayPlans: DayPlansMap): DayPlan | undefined {
...
export function validateImportData(jsonString: string): { valid: boolean; data?: StorageData; error?: string } {
  try {
    ...
    if (!data.dayPlans || typeof data.dayPlans !== 'object') {
      return { valid: false, error: 'Invalid data format: missing or invalid dayPlans' };
    }
    ...
    for (const [date, plan] of Object.entries(data.dayPlans)) {
      ...
      if (!plan || typeof plan !== 'object') {
        return { valid: false, error: 'Invalid data format: invalid plan object' };
      }
      const dayPlan = plan as any;
      if (!Array.isArray(dayPlan.tasks)) {
        return { valid: false, error: 'Invalid data format: tasks must be an array' };
      }
    }
